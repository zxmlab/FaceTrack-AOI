...
def get_bounds_from_row(row, csv_data):
    ...
    frame_index = row['VIDEO_FRAME_INDEX']
    bounds_dict = {}
...
def evaluate_gaze_regions(df, csv_data, y_adjust):
    ...
    for idx, row in df.iterrows():
        x, y = row['RIGHT_GAZE_X'], row['RIGHT_GAZE_Y']
...
def process_gaze_video(input_txt_path, input_CSVtable_dir, videofilter='.mp4', output_csv_path=None, y_adjust=60):
    print(f"Reading gaze data from: {input_txt_path}")
    df = pd.read_csv(input_txt_path, sep='\t', na_values='.')

    unique_video_names = df[
        df["VIDEO_NAME"].notna() & df["VIDEO_NAME"].str.endswith(videofilter)
    ]["VIDEO_NAME"].unique()
    ...
    print("Evaluating gaze regions...")
    df = evaluate_gaze_regions(df, csv_data, y_adjust)
