...
import cv2
import os
from main_module import scale_shape


def in_bounds(x, y, bounds, xscale=1.0, yscale=1.0):
    if bounds is None or len(bounds) < 3:
        return False, 0
    bounds = bounds.copy()
    bounds[::, 1] += 60  # Adjust for y-axis
...
def get_bounds_from_row(row, csv_data):
    ...
    if video_df is not None:
        ...
        if match.size:
            ...
            print(f"[Info] No matching landmark data for video '{video_name}', frame {frame_index}")

    return bounds_dict


def evaluate_fixation_regions(df, csv_data):
        ...
        try:
            ...
            for region, xscale, yscale in region_params:
                ...
                if region_bounds is not None:
                    inside, area = in_bounds(x, y, region_bounds, xscale, yscale)
...
def process_fixation_video(input_txt_path, input_CSVtable_dir, videofilter='.mp4', output_csv_path=None):
    ...
    df = evaluate_fixation_regions(df, csv_data)
